...
function M.setup()
	...
	utils.map("n", "<leader>fp", "<cmd>lua require('telescope').extensions.projects.projects{}<CR>", { desc = "Find projects" })
	utils.map("n", "<leader>fb", ":Telescope buffers<CR>", { desc = "Find buffers" })
	utils.map("n", "<leader>fs", "<cmd>lua require'telescope.builtin'.symbols{ sources = {'emoji'} }<CR>", { desc = "Find emoji" })
